...
namespace MOHO.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public IActionResult Doanhthu(DateTime? ngay)
        {
            if (ngay.HasValue)
            {
                // Lấy doanh thu theo ngày
                var doanhThuTheoNgay = ThongKeDoanhThuTheoNgayDat(ngay.Value);

                // Lấy tháng và năm từ ngày
                int thang = ngay.Value.Month;
                int nam = ngay.Value.Year;

                // Lấy doanh thu theo tháng
                var doanhThuTheoThang = ThongKeDoanhThuTheoThang(thang, nam);

                // Lấy doanh thu theo năm
                var doanhThuTheoNam = ThongKeDoanhThuTheoNam(nam);

                // Lưu vào ViewBag để hiển thị trong View
        ...
        private decimal ThongKeDoanhThuTheoNgayDat(DateTime ngayDat)
        {
            return _context.ChiTietDonHangs
                .Where(m => m.Status == (int?)1 && m.MadonNavigation.Ngaydat.HasValue && m.MadonNavigation.Ngaydat.Value.Date == ngayDat)
                .Sum(n => (n.Soluong.HasValue && n.Gia.HasValue) ? (decimal)n.Soluong.Value * n.Gia.Value : 0);
        }

        private decimal ThongKeDoanhThuTheoThang(int thang, int nam)
        {
            return _context.ChiTietDonHangs
                .Where(m => m.Status == (int?)1 && m.MadonNavigation.Ngaydat.HasValue
                    && m.MadonNavigation.Ngaydat.Value.Month == thang
                    && m.MadonNavigation.Ngaydat.Value.Year == nam)
                .Sum(n => (n.Soluong.HasValue && n.Gia.HasValue) ? (decimal)n.Soluong.Value * n.Gia.Value : 0);
        }

        private decimal ThongKeDoanhThuTheoNam(int nam)
        {
            return _context.ChiTietDonHangs
                .Where(m => m.Status == (int?)1 && m.MadonNavigation.Ngaydat.HasValue
                    && m.MadonNavigation.Ngaydat.Value.Year == nam)
